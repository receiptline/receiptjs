...
const Receipt = (() => {
    ...
    ['Courier+Prime', 'BIZ+UDGothic', 'Sarabun'].forEach(family => {
    ...
    const Base64PNG = {
        async from(svg, options = {}) {
            ...
            const width = parseInt(doc.documentElement.getAttribute('width'));
            const height = parseInt(doc.documentElement.getAttribute('height'));
            const canvas = document.createElement('canvas');
            const context = canvas.getContext('2d');
            if (options.landscape) {
                canvas.width = height;
                canvas.height = width + m + n;
                context.translate(0, width + n);
            ...
            const lang = group.getAttribute('xml:lang').toLowerCase() || 'en';
            if (!/^(ko|zh)/.test(lang)) {
                const font = group.getAttribute('font-family');
                const size = group.getAttribute('font-size');
                const color = group.getAttribute('fill');
                ...
                // <svg><g><g>
                group.querySelectorAll('g').forEach(g => {
                    const [x0, y0] = g.getAttribute('transform').match(/translate\((\d+),(\d+)\)/).slice(1).map(Number);
                    // <svg><g><g><text>
                    g.querySelectorAll('text').forEach(text => {
                        const y1 = text.hasAttribute('y') ? Number(text.getAttribute('y')) : 0;
                        const ul = text.hasAttribute('text-decoration');
                        const em = text.hasAttribute('stroke');
                        const iv = text.hasAttribute('filter');
                        const [sx, sy] = text.hasAttribute('transform') ? text.getAttribute('transform').match(/scale\((\d),(\d)\)/).slice(1).map(Number) : [1, 1];
                        // <svg><g><g><text><tspan>
                        text.querySelectorAll('tspan').forEach(tspan => {
                            const x = x0 / sx + Number(tspan.getAttribute('x'));
                            const y = y0 / sy + y1;
                            const t = tspan.textContent;
                            const w = c * commands.svg.measureText(t, encoding[lang] || 'multilingual');
                            const h = c + c;
                            const u = /^ja/.test(lang) ? 0 : 2;
                            context.save();
                            context.scale(sx, sy);
                            if (iv) {
                            ...
                            if (ul) {
                                context.fillRect(x - w / 2, y - u, w, -1);
                                if (em) {
                                    context.strokeRect(x - w / 2, y - u , w, -1);
                                }
                            }
                            context.restore();
                        });
            ...
            const img = new Image();
            img.src = 'data:image/svg+xml,' + encodeURIComponent(new XMLSerializer().serializeToString(doc));
            await img.decode();
