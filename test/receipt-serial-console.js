...
const ReceiptSerial = (() => {
    ...
    const serialport = () => {
        ...
        return {
            ...
            open(options) {
                // select port
                navigator.serial.requestPort().then(async p => {
                    port = p;
                    rfcomm = 'bluetoothServiceClassId' in port.getInfo();
                    // open port
                    for (let i = 0; i < 3; i++) {
                        try {
                    ...
                    while (opened && port.readable) {
                        ...
                        try {
                            while (opened) {
                                ...
                                if (enabled) {
                                    console.log(new Date().toISOString(), 'read:', value);
            ...
            async write(data, encoding) {
                if (opened && port.writable) {
                    const buffer = encoding !== 'binary' ? new TextEncoder().encode(data) : Uint8Array.from(data, c => c.charCodeAt(0));
                    if (rfcomm) {
                        try {
                            await writer.ready;
                            await writer.write(buffer);
                            console.log(new Date().toISOString(), 'write:', buffer);
                        }
                        catch (e) {
                        ...
                        for (let i = 0; i < buffer.length; i += 1024) {
                            const count = Math.min(1024, buffer.length - i);
                            const chunk = buffer.subarray(i, i + count);
                            while (opened) {
                                try {
                                    ...
                                    if (signals.clearToSend) {
                                        ...
                                        console.log(new Date().toISOString(), 'write:', chunk);
                                        break;
                                    }
                                    else {
                                        console.log(new Date().toISOString(), 'write: busy');
                                        await new Promise(resolve => setTimeout(resolve, 100));
                                    }
                                }
                                catch (e) {
    ...
    const command = {
        ...
        escptr: '\x10\x04\x02', // DLE EOT n
        escdrw: '\x10\x04\x01', // DLE EOT n
        escclr: '\x10\x14\x08\x01\x03\x14\x01\x06\x02\x08', // DLE DC4 n d1 d2 d3 d4 d5 d6 d7
    ...
    return {
        ...
        connect(options = {}) {
            ...
            const update = newstatus => {
                if (newstatus !== status) {
                    ...
                    if (status === state.print) {
                        ...
                        console.log(new Date().toISOString(), 'print', newstatus === state.online ? 'success' : newstatus);
                    }
                    // status event
                    status = newstatus;
            ...
            conn.on('open', async () => {
                // hello to printer
                drain = await conn.write(command.hello, 'binary');
                // set timer
                timeout = setTimeout(async () => {
                    // buffer clear
                    drain = await conn.write('\x00'.repeat(65536) + command.hello, 'binary');
                    // set timer
                    timeout = setTimeout(async () => {
                        // buffer clear
                        drain = await conn.write('\x00'.repeat(65536) + command.hello, 'binary');
                    ...
                    switch (printer) {
                        case '':
                            if ((buffer[0] & 0xf0) === 0xb0) {
                                ...
                                console.log(new Date().toISOString(), 'sii: initialized response');
                                // clear data
                                buffer.shift();
                                // clear timer
                                clearTimeout(timeout);
                                ...
                                drain = await conn.write(command.siiasb, 'binary');
                            }
                                ...
                                if (len > 1) {
                                    ...
                                    if (l <= len) {
                                        ...
                                        console.log(new Date().toISOString(), `printer: ${printer}`);
                                        // enable automatic status
                                        drain = await conn.write(command.starasb, 'binary');
                                    }
                                }
                            }
                            else if ((buffer[0] & 0x93) === 0x12) {
                                // escpos: realtime status
                                console.log(new Date().toISOString(), 'escpos: realtime status');
                                ...
                                console.log(new Date().toISOString(), `printer: ${printer}`);
                                // get drawer status
                                drain = await conn.write(command.escdrw, 'binary');
                            }
                        ...
                        case 'sii':
                            if ((buffer[0] & 0xf0) === 0x80) {
                                // sii: status
                                console.log(new Date().toISOString(), 'sii: status');
                                ...
                                // sii: other
                                console.log(new Date().toISOString(), 'sii: other');
                                buffer.shift();
                            }
                            break;
                        ...
                        case 'star':
                            if ((buffer[0] & 0xf1) === 0x21) {
                                ...
                                // star: other
                                console.log(new Date().toISOString(), 'star: other');
                                buffer.shift();
                            }
                            break;

                        case 'escpos':
                            if ((buffer[0] & 0x93) === 0x12) {
                                // escpos: realtime status
                                // cash drawer
                                ...
                                if (status !== state.offline) {
                                    ...
                                    timeout = setTimeout(async () => {
                                        // get printer status
                                        drain = await conn.write(command.escptr, 'binary');
                                    }, 3000);
                                }
                                else {
                                    // printer control language
                                    ...
                                    drain = await conn.write(command.escasb, 'binary');
                                    // set timer
                                    timeout = setTimeout(async () => {
                                        // buffer clear
                                        drain = await conn.write(command.escclr, 'binary');
                                        // set timer
                                        timeout = setTimeout(async () => {
                                            // buffer clear
                                            drain = await conn.write('\x00'.repeat(65536) + command.escclr, 'binary');
                                        }, 3000);
                                    }, 3000);
                                }
                            }
                        ...
                        default:
                            ...
                            if ((buffer[0] & 0x90) === 0) {
                                // escpos: status
                                console.log(new Date().toISOString(), 'escpos: status');
                                if (status === state.print && drain) {
                                    // online event
                                    update(state.online);
                                }
                                ...
                                if (i > 0) {
                                    ...
                                    if (block[0] === 0x5f) {
                                        ...
                                        // power on
                                        if (block[1] === 0x31) {
                                            console.log(new Date().toISOString(), 'escpos: power on');
                                            // printer control language
                                            printer = '';
                                            console.log(new Date().toISOString(), `printer: ${printer}`);
                                            // hello to printer
                                        ...
                                        // buffer clear
                                        if (block[1] === 0x25) {
                                            console.log(new Date().toISOString(), 'escpos: buffer clear');
                                            // clear timer
                                            clearTimeout(timeout);
                                            // get model info and enable automatic status
                                            drain = await conn.write(command.escasb, 'binary');
            ...
            // open port
            conn.open({ ...{ baudRate: 115200, flowControl: 'hardware', bufferSize: 2048 }, ...options });
                ...
                print(markdown, options) {
                    ...
                    return new Promise(async res => {
                        ...
                        if (status === state.online) {
                            ...
                            const command = await Receipt.from(markdown, `-p ${printer} ${options}`).toCommand();
                            // write command
                            if (/^star$/.test(printer)) {
                                // star
                                drain = await conn.write(command
                                    .replace(/^(\x1b@)?\x1b\x1ea\x00/, '$1\x1b\x1ea\x01')  // (ESC @) ESC RS a n
                                    .replace(/(\x1b\x1d\x03\x01\x00\x00\x04?|\x1b\x06\x01)$/, '\x17'), 'binary'); // ETB
                            }
                            else {
                                // escpos
                                drain = await conn.write(command.replace(/^\x1b@\x1da\x00/, '\x1b@\x1da\xff'), 'binary'); // ESC @ GS a n
                            }
                        }
                        else {
                            // print response
